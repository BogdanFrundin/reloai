      ...
      >
        <div className="flex w-full items-start justify-between gap-3">
          ...
          <div className="flex flex-shrink-0 items-center gap-2">
            ...
            <span className={`text-text-muted transition-transform duration-150 ${open ? "rotate-180" : ""}`}>
              {CHEVRON_ICON}
            </span>
          </div>
        </div>
        <div>
          ...
          {guide.description && (
            <p className={`mt-1 text-xs text-text-muted ${open ? "" : "line-clamp-3"}`}>{guide.description}</p>
          )}
        </div>
      </button>
