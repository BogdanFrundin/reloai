...
function DistrictCard({ district, t }: { district: WarsawDistrict; t: Dictionary["housing"] }) {
  ...
  return (
    ...
    >
      ...
      <p className="mt-2 min-h-7 bg-gradient-to-br from-text-primary to-text-muted bg-clip-text text-xl font-bold text-transparent">
        {district.priceMin.toLocaleString()}–{district.priceMax.toLocaleString()} PLN
      </p>
      <p className="mt-2 line-clamp-3 min-h-[60px] text-sm text-text-secondary">{description}</p>
      <span className="mt-3 inline-flex w-fit min-h-7 items-center rounded-full border border-border-subtle bg-surface-1 px-2.5 py-1 text-[11px] text-text-muted">
