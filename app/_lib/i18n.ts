...
export type Dictionary = {
  ...
  onboarding: {
    ...
    steps: {
      ...
      kazakhstanScenario: { question: string; subheading: string };
      tajikistanScenario: { question: string; subheading: string };
      // Shared follow-up step, asked once regardless of citizenship —
      // "Уже в Польше" branch of any of the 7 above splits into this single
      // yes/no question ("do you already hold a karta pobytu or valid visa
      // D?") instead of duplicating the same question 7 times. See
      // app/onboarding/page.tsx's "scenarioStatus" step and the
      // SCENARIO_ANSWER_FIELD_BY_CITIZENSHIP lookup that resolves which
      // country-specific *_scenario field it actually writes into.
      scenarioStatus: { question: string; subheading: string };
    ...
    // Now a 2-way choice (self vs "already in Poland") per the ТЗ Логика
    // "Уже в Польше" spec — the further already_status/already_no_status
    // split moved to the shared scenarioStatusOptions below.
    belarusScenarioOptions: { self: string; already: string };
    georgiaScenarioOptions: { self: string; already: string };
    moldovaScenarioOptions: { self: string; already: string };
    uzbekistanScenarioOptions: { self: string; already: string };
    turkeyScenarioOptions: { self: string; already: string };
    kazakhstanScenarioOptions: { self: string; already: string };
    tajikistanScenarioOptions: { self: string; already: string };
    scenarioStatusOptions: { yes: string; no: string };
    results: {
      heading: string;
      loading: string;
      selectButton: string;
...
export const dictionaries: Record<Lang, Dictionary> = {
  en: {
    ...
    housing: {
      ...
      topDistrictDescs: {
        mokotow: "One of the largest and most diverse districts: prestigious Old Mokotów with its villas sits alongside business Służewiec, where international companies concentrate their offices. Developed infrastructure, metro, parks and restaurants for every taste.",
        wola: "The most dynamic business district of recent years — skyscrapers at Rondo Daszyńskiego and Warsaw's new business center. Yet quiet residential quarters and historic buildings remain, with metro connecting to the center in minutes.",
        zoliborz: "Green and quiet district in the north center with pre-war buildings and parks. One of Warsaw's most atmospheric districts, metro nearby, prices above average due to location.",
        ochota: "Adjacent to downtown but noticeably cheaper. Many residential blocks from the PRL era, developed infrastructure, near Warsaw University and Polytechnic — popular with students and young families.",
        srodmiescie: "Historic and business heart of Warsaw — Palace of Culture, skyscrapers, museums and restaurants within walking distance. Highest prices in the city, but everything is at hand with excellent transport access.",
        ursynow: "Large southern district, mostly multi-story buildings but with wide boulevards and parks; near SGGW campus, many students and young families. Good connection to center — end station of metro M1.",
        "praga-polnoc": "Historic, authentic Praga — crumbling facades sit alongside trendy lofts and art spaces. Metro nearby, close to center, growing investor interest and creative youth.",
        "praga-poludnie": "Warsaw's most populated dzielnica on the right bank of the Vistula. Varied development — from old Grochów houses to new residential complexes near metro; more affordable prices and lively local life.",
        bemowo: "Residential district in the northwest with metro and large Bemowo Park. Many new developments in recent years, quieter than center, convenient for families with children.",
        bielany: "Residential district in the northwest with its own metro branch (M1), plenty of greenery and Kampinos Forest nearby. Quieter than center, actively building new housing, popular with families.",
        targowek: "District on the right bank of the Vistula, mix of Soviet-era panel buildings and new residential complexes. Metro and trams, prices lower than downtown, developing infrastructure.",
        bialoleka: "Largest new construction zone in the north of the city, cheapest prices in Warsaw. Metro hasn't arrived yet but is under construction, infrastructure developing with the district.",
        wilanow: "Prestigious southern district near Wilanów Palace and Park — new construction, families with children, developed infrastructure. No metro, but good bus and tram connections.",
        wlochy: "District near Chopin Airport in the southwest, mix of industrial zones and new residential complexes. Good rail connections (SKM), moderate prices.",
        rembertow: "Small quiet district in the east of the city, plenty of greenery and private housing. Further from center but noticeably cheaper and quieter.",
        wawer: "Warsaw's largest district by area, southeastern outskirts — private houses, pine forests, dacha atmosphere. Weaker public transport, but plenty of space and quiet for affordable prices.",
        wesola: "Smallest and most remote district on the east side of the city, almost a separate village — low-rise buildings, greenery, minimal noise. Longer commute to center, but prices and peace make up for it.",
        ursus: "Former industrial district in the west of the city, now actively built with new residential complexes on the site of the tractor factory. Good rail connection with center, plenty of greenery, peaceful atmosphere.",
      },
    ...
    banks: {
      ...
      whichBankDescription: "Ask AI",
      whichBankLearnMore: "Get a recommendation",
      whichBankGuide: { heading: "Which bank should I choose?" },
      moreAboutBank: "Details",
      moreFiltersBtn: "More",
      byReviews: "by reviews",
      allBanksTemplate: "All banks ({n})",
      visitStatusOnline: "Fully online",
      visitStatusBranch: "Branch visit required",
      visitStatusCourier: "Courier delivery",
      visitStatusOnlineIfId: "Online with a Polish ID",
    ...
    medicine: {
      ...
      usefulSites: [
        { url: "znanylekarz.pl", desc: "Book a doctor online — Russian-speaking doctors available" },
        { url: "ewus.nfz.gov.pl", desc: "Check your NFZ insurance status" },
        { url: "nfz.gov.pl", desc: "Official NFZ website" },
        { url: "pacjent.gov.pl", desc: "Patient portal (IKP): doctor appointments, e-prescriptions, test results online" },
      ],
      ...
      dentalChains: "Recommended chains: DentalPro, Medicover Stomatologia",
    ...
    guideCard: {
      ...
      headlines: {
        ...
        forForeigners: "Foreigner-friendly",
      },
      tagDescriptions: {
      ...
      statOpeningCost: "Account opening",
      bankHighlights: {
        ...
        "Plus Bank": "Focus on remote banking service",
    ...
    onboarding: {
      ...
      steps: {
        ...
        kazakhstanScenario: { question: "Which of these applies to you?", subheading: "There's no visa-free entry to Poland for Kazakhstan citizens at all, and the work goal needs a full zezwolenie na pracę (not the fast oświadczenie track) — applications go directly to the embassy in Astana or consulate in Almaty (no VFS Global), but a high-demand lottery can gate the appointment and processing itself takes a minimum 30 working days. The right track depends on where you are in the process." },
        tajikistanScenario: { question: "Which of these applies to you?", subheading: "Poland has no diplomatic mission in Tajikistan at all — every visa (Schengen C or national D), for every goal, has to be filed in person in Tashkent, Uzbekistan, since the Polish ambassador there is also accredited to Tajikistan. The work goal also needs a full zezwolenie na pracę (oświadczenie was never available to Tajik citizens). The right track depends on where you are in the process." },
        scenarioStatus: { question: "Do you already hold a karta pobytu or a valid visa D?", subheading: "If yes, you'll go through the normal flow for renewing or updating your documents. If no, we'll skip the goal question and show you 2 ready-made paths to legal status without leaving Poland." },
      ...
      belarusScenarioOptions: {
        ...
        already: "I'm already in Poland",
      },
      georgiaScenarioOptions: {
        self: "I'm relocating on my own from Georgia",
        already: "I'm already in Poland",
      },
      moldovaScenarioOptions: {
        self: "I'm relocating on my own from Moldova",
        already: "I'm already in Poland",
      },
      ...
      kazakhstanScenarioOptions: {
        self: "I'm relocating on my own from Kazakhstan",
        already: "I'm already in Poland",
      },
      tajikistanScenarioOptions: {
        self: "I'm relocating on my own from Tajikistan",
        already: "I'm already in Poland",
      },
      scenarioStatusOptions: {
        yes: "Yes, I hold a karta pobytu or valid visa D",
        no: "No, just a short-stay visa or visa-free entry — no status yet",
      },
      results: {
  ...
  ru: {
    ...
    housing: {
      ...
      topDistrictDescs: {
        mokotow: "Один из крупнейших и самых разных районов: престижный Старый Мокотув с виллами соседствует с деловым Służewcem, где сосредоточены офисы международных компаний. Развитая инфраструктура, метро, парки и рестораны на любой вкус.",
        wola: "Самый динамичный деловой район последних лет — небоскрёбы у Rondo Daszyńskiego и новый деловой центр Варшавы. При этом сохранились тихие жилые кварталы и историческая застройка, метро связывает с центром за несколько минут.",
        zoliborz: "Зелёный и спокойный район на севере центра с довоенной застройкой и парками. Один из самых атмосферных районов Варшавы, метро рядом, цены выше среднего из-за расположения.",
        ochota: "Граничит со Средместьем, при этом заметно дешевле. Много кварталов застройки времён ПНР, развитая инфраструктура, рядом Варшавский университет и Политехника — популярно у студентов и молодых семей.",
        srodmiescie: "Исторический и деловой центр Варшавы — Дворец культуры, небоскрёбы, музеи и рестораны в шаговой доступности. Самые высокие цены в городе, зато всё под рукой и отличная транспортная доступность.",
        ursynow: "Большой южный район, застроен в основном многоэтажками, но с широкими бульварами и парками; рядом кампус SGGW, много студентов и молодых семей. Хорошая связь с центром — конечная станция метро M1.",
        "praga-polnoc": "Историческая, самая настоящая Прага — обшарпанные фасады соседствуют с модными лофтами и арт-пространствами. Метро, близость к центру, растущий интерес инвесторов и творческой молодёжи.",
        "praga-poludnie": "Самая населённая дзельница Варшавы, на правом берегу Вислы. Разная застройка — от старых домов Грохова до новых жилых комплексов у метро; более демократичные цены и оживлённая местная жизнь.",
        bemowo: "Спальный район на северо-западе с метро и большим парком Бемово. Много новостроек последних лет, спокойнее центра, удобен для семей с детьми.",
        bielany: "Спальный район на северо-западе со своей веткой метро (M1), много зелени и Кампиноская пуща рядом. Спокойнее центра, активно строится новое жильё, популярен у семей.",
        targowek: "Район на правом берегу Вислы, смесь панельной застройки советского времени и новых жилых комплексов. Метро и трамваи, цены ниже, чем в центре, развивающаяся инфраструктура.",
        bialoleka: "Крупнейшая зона новостроек на севере города, самые низкие цены в Варшаве. Метро пока не дошло, но строится, инфраструктура развивается вместе с районом.",
        wilanow: "Престижный южный район рядом с Вилянувским дворцом и парком — новая застройка, семьи с детьми, развитая инфраструктура. Метро нет, но хорошее автобусное и трамвайное сообщение.",
        wlochy: "Район рядом с аэропортом Шопена на юго-западе города, смесь промышленных зон и новых жилых комплексов. Хорошее железнодорожное сообщение (SKM), цены умеренные.",
        rembertow: "Небольшой тихий район на востоке города, много зелени и частной застройки. Дальше от центра, зато заметно дешевле и спокойнее.",
        wawer: "Самый большой по площади район Варшавы, юго-восточная окраина — частные дома, сосновые леса, дачная атмосфера. Общественный транспорт слабже, зато простор и тишина за небольшие деньги.",
        wesola: "Самый маленький и удалённый район на востоке города, почти отдельный посёлок — низкая застройка, зелень, мало шума. Добираться до центра дольше, но цены и спокойствие компенсируют.",
        ursus: "Бывший промышленный район на западе города, сейчас активно застраивается новыми жилыми комплексами на месте завода тракторов. Хорошее ж/д сообщение с центром, много зелени, спокойная атмосфера.",
      },
    ...
    banks: {
      ...
      otherBanksLabel: "Полный список рейтинга",
      ...
      whichBankDescription: "Спросить ИИ",
      whichBankLearnMore: "Получить рекомендацию",
      whichBankGuide: { heading: "Какой банк выбрать?" },
      moreAboutBank: "Подробнее",
      moreFiltersBtn: "Ещё",
      ...
      visitStatusCourier: "Доставка курьером",
      visitStatusOnlineIfId: "Онлайн с польским ID",
      referralBonusLabel: "Бонус за друга: {amount}",
      branchesByCityLabel: "Отделения по городам",
      usefulPhrasesLabel: "Полезные фразы",
    ...
    medicine: {
      ...
      usefulSites: [
        { url: "znanylekarz.pl", desc: "Запись к врачу онлайн — есть русскоязычные врачи" },
        { url: "ewus.nfz.gov.pl", desc: "Проверить свою страховку NFZ" },
        { url: "nfz.gov.pl", desc: "Официальный сайт NFZ" },
        { url: "pacjent.gov.pl", desc: "Личный кабинет пациента (IKP): записи на приём, электронные рецепты, результаты анализов онлайн" },
      ],
      ...
      dentalChains: "Рекомендуемые сети: DentalPro, Medicover Stomatologia",
    ...
    guideCard: {
      ...
      headlines: {
        ...
        forForeigners: "Лоялен к иностранцам",
      },
      tagDescriptions: {
      ...
      statClients: "Клиенты",
      statBranches: "Отделения",
      statOpeningCost: "Открытие счёта",
      bankHighlights: {
        ...
        "Plus Bank": "Упор на дистанционное обслуживание",
    ...
    onboarding: {
      ...
      steps: {
        ...
        kazakhstanScenario: { question: "Какой из вариантов вам подходит?", subheading: "Безвизового въезда в Польшу для граждан Казахстана нет вообще, а для цели «работа» нужно полное zezwolenie na pracę (без быстрой процедуры oświadczenie) — документы подаются напрямую в посольство в Астане или консульство в Алматы (визового центра нет), но при высоком спросе запись идёт через жеребьёвку, а само рассмотрение занимает от 30 рабочих дней. Нужный маршрут зависит от того, на каком этапе вы уже находитесь." },
        tajikistanScenario: { question: "Какой из вариантов вам подходит?", subheading: "У Польши вообще нет дипломатического представительства в Таджикистане — любая виза (шенгенская C или национальная D), для любой цели, подаётся лично только в Ташкенте, Узбекистан, так как польский посол там аккредитован и на Таджикистан тоже. Для цели «работа» нужно полное zezwolenie na pracę — процедура oświadczenie для граждан Таджикистана никогда не была доступна. Нужный маршрут зависит от того, на каком этапе вы уже находитесь." },
        scenarioStatus: { question: "У вас уже есть карта побыту или действующая виза D?", subheading: "Если да — дальше обычный маршрут по продлению или обновлению документов. Если нет — вопрос о цели переезда зададим позже: сразу покажем 2 готовых пути легализации без выезда из Польши." },
      ...
      moldovaScenarioOptions: {
        self: "Переезжаю самостоятельно из Молдовы",
        already: "Я уже в Польше",
      },
      uzbekistanScenarioOptions: {
      ...
      kazakhstanScenarioOptions: {
        self: "Переезжаю самостоятельно из Казахстана",
        already: "Я уже в Польше",
      },
      tajikistanScenarioOptions: {
        self: "Переезжаю самостоятельно из Таджикистана",
        already: "Я уже в Польше",
      },
      scenarioStatusOptions: {
        yes: "Да, есть карта побыту или действующая виза D",
        no: "Нет, только краткосрочная виза или безвиз — статуса ещё нет",
      },
      results: {
  ...
  uz: {
    ...
    housing: {
      ...
      topDistrictDescs: {
        mokotow: "Eng katta va turli xil tumanlardan biri: prestizhli Eski Mokotuw villaları bilan bir qatorada delovoy Služevec bor, bu yerda xalqaro kompaniyalarning ofislari joylashgan. Rivojlangan infratuzilma, metro, parklar va har qanday ta'mga mos restoranlar.",
        wola: "So'nggi yillarning eng dinamik delovoy tumani — Rondo Daszyńskomu qarshi ko'kkaza qurilishlar va Varshavaning yangi delovoy markazi. Shu bilan birga, tinch turar-joy kvartallar va tarixiy qurilishlar saqlanib qolgan, metro markazga bir necha minutda bog'lanadi.",
        zoliborz: "Markazning shimoli tarafida ko'kalamzor va tinch tuman, so'ng tuzilgan binolar va parklar bilan. Varshavaning eng atmosferal tumanlaridan biri, metro yaqin, joylashuvi sababli narxlar o'rtacha yuqori.",
        ochota: "Markazning qoshida turadi, lekin ancha arzonroq. Sovet davridagi ko'plab turar-joy kvartallari, rivojlangan infratuzilma, Varshava universiteti va Politexnika yaqin — talabalar va yosh oilalar orasida mashhur.",
        srodmiescie: "Varshavaning tarixiy va delovoy markazi — Madaniyat saroyи, ko'kkaza qurilishlar, muzeylar va restoranlar yurish masofasida. Shaharning eng yuqori narxlari, lekin hamma narsa qo'lda va ajoyib transport bog'lanishi.",
        ursynow: "Katta janubiy tuman, asosan ko'p qavatli binolar, lekin keng bulvarlar va parklar bor; SGGW kampusi yaqinida, ko'p talabalar va yosh oilalar. Markazga yaxshi aloqa — M1 metro stantsiyasining oxirgi stantsiyasi.",
        "praga-polnoc": "Tarixiy, haqiqiy Praga — xarobalar egilgan fasadlar zamonaviy loft va shunshulik bilandir. Metro yaqin, markazga yaqin, investorlar va ijodiy yoshlarning qiziquvchi o'sishi.",
        "praga-poludnie": "Varshavaning eng ko'p aholi soni bo'lgan dzhelnica, Vistulaning o'ng sahili. Turli xil qurilish — eski Groxov uylaridan metro yaqinida yangi turar-joy komplekslargacha; ancha arzon narxlar va jonli mahalliy hayot.",
        bemowo: "Shimoli-g'arbdagi turar-joy tumani, metro va katta Bemovo parki bilan. So'nggi yillarning ko'plab yangi qurilishlari, markazdan tinchroq, bolali oilalar uchun qulay.",
        bielany: "Shimoli-g'arbdagi turar-joy tumani o'z metro shoxlari (M1) bilan, ko'p yashilliklar va Kampinos o'rmoni yaqin. Markazdan tinchroq, faol ravishda yangi uylar qurilmoqda, oilalar orasida mashhur.",
        targowek: "Vistulaning o'ng sahilida tuman, Sovet davridagi panel binolari va yangi turar-joy komplekslari aralashmasi. Metro va tramvaylar, markazccha qaraganda arzonroq narxlar, rivojlanayotgan infratuzilma.",
        bialoleka: "Shaharning shimolida eng katta yangi qurilish zonasi, Varshavada eng arzon narxlar. Metro hali kelmagan, lekin qurilmoqda, infratuzilma tuman bilan rivojlanmoqda.",
        wilanow: "Vilyanuv saroyи va parki yaqinida prestizhli janubiy tuman — yangi qurilish, bolali oilalar, rivojlangan infratuzilma. Metro yo'q, lekin yaxshi avtobas va tramvay bog'lanishi.",
        wlochy: "Shopin aeroportи yaqinida janub-g'arbdagi tuman, sanoat zonalari va yangi turar-joy komplekslari aralashmasi. Yaxshi temir yo'l bog'lanishi (SKM), o'rtacha narxlar.",
        rembertow: "Shaharning sharqida kichik tinch tuman, ko'p yashilliklar va shaxsiy uylar. Markazdan uzoqroq, lekin ancha arzonroq va tinchroq.",
        wawer: "Varshavaning maydon bo'yicha eng katta tumani, janub-sharqiy chegara — shaxsiy uylar, qarag'ay o'rmanlari, dacha atmosferi. Zaif umumiy transport, lekin arzon narxdagi ko'p maydoni va jimliklar.",
        wesola: "Shaharning sharqida eng kichik va masofaviy tuman, deyarli alohida qishloq — past qurilishlar, yashilliklar, minimal shovqin. Markazga uzoq marshut, lekin narxlar va tinchlik to'ldiradi.",
        ursus: "Shaharning g'arbida sobiq sanoat tumani, hozir traktor zavodi o'rnida yangi turar-joy komplekslari bilan faol ravishda qurilmoqda. Markazga yaxshi temir yo'l bog'lanishi, ko'p yashilliklar, tinch atmosfera.",
      },
    ...
    banks: {
      ...
      otherBanksLabel: "To'liq reyting",
      ...
      whichBankDescription: "AI'dan so'rang",
      whichBankLearnMore: "Tavsiya olish",
      whichBankGuide: { heading: "Qaysi bankni tanlash kerak?" },
      moreAboutBank: "Batafsil",
      moreFiltersBtn: "Yana",
      ...
      visitStatusCourier: "Kuryer orqali yetkazish",
      visitStatusOnlineIfId: "Polyak ID bilan onlayn",
      referralBonusLabel: "Do'stni taklif qiling: {amount}",
      branchesByCityLabel: "Shahar bo'yicha filiallar",
      usefulPhrasesLabel: "Foydali iboralar",
    ...
    medicine: {
      ...
      usefulSites: [
        { url: "znanylekarz.pl", desc: "Shifokorga onlayn yozilish — rus tilida so'zlashuvchi shifokorlar bor" },
        { url: "ewus.nfz.gov.pl", desc: "NFZ sug'urtangizni tekshirish" },
        { url: "nfz.gov.pl", desc: "NFZ rasmiy sayti" },
        { url: "pacjent.gov.pl", desc: "Bemor portali (IKP): shifokorga yozilish, elektron retseptlar, tahlillar natijalari onlayn" },
      ],
      ...
      dentalChains: "Tavsiya etilgan tarmoqlar: DentalPro, Medicover Stomatologia",
    ...
    guideCard: {
      ...
      tagsLabel: "Teglar",
      tags: { noPesel: "PESELsiz", fullyOnline: "To'liq onlayn", free: "Bepul", multicurrency: "Ko'p valyutali", forForeigners: "Xorijliklar uchun" },
      headlines: {
        noPesel: "PESELsiz",
        fullyOnline: "Hisobni onlayn ochish",
        free: "Bepul xizmat",
        multicurrency: "Ko'p valyutali hisob",
        forForeigners: "Xorijliklarga qulay",
      },
      tagDescriptions: {
      ...
      statBranches: "Filiallar",
      statOpeningCost: "Hisob ochish narxi",
      bankHighlights: {
        ...
        "Plus Bank": "Masofaviy xizmatga urg'u",
    ...
    onboarding: {
      ...
      steps: {
        ...
        kazakhstanScenario: { question: "Bularning qaysi biri sizga tegishli?", subheading: "Qozog'iston fuqarolari uchun Polshaga vizasiz kirish umuman yo'q, \"ish\" maqsadi uchun esa tezkor oświadczenie tartibisiz to'liq zezwolenie na pracę kerak — hujjatlar bevosita Astanadagi elchixona yoki Almatidagi konsullikka topshiriladi (VFS Global yo'q), lekin yuqori talab bo'lganda qur'a orqali navbat belgilanadi va ko'rib chiqish kamida 30 ish kuni davom etadi. Kerakli yo'l qaysi bosqichda ekaningizga bog'liq." },
        tajikistanScenario: { question: "Bularning qaysi biri sizga tegishli?", subheading: "Polshaning Tojikistonda umuman diplomatik vakolatxonasi yo'q — har qanday viza (Shengen C yoki milliy D), har qanday maqsad uchun, faqat shaxsan Toshkentda, O'zbekistonda topshiriladi, chunki u yerdagi Polsha elchisi Tojikistonga ham akkreditatsiya qilingan. \"Ish\" maqsadi uchun ham to'liq zezwolenie na pracę kerak — Tojikiston fuqarolari uchun oświadczenie tartibi hech qachon mavjud bo'lmagan. Kerakli yo'l qaysi bosqichda ekaningizga bog'liq." },
        scenarioStatus: { question: "Sizda allaqachon karta pobytu yoki amaldagi D viza bormi?", subheading: "Agar ha bo'lsa, hujjatlaringizni yangilash yoki uzaytirish bo'yicha odatiy yo'l bilan davom etasiz. Agar yo'q bo'lsa, maqsad haqidagi savolni keyinroq beramiz — Polshadan chiqmasdan legallashish uchun 2 tayyor yo'lni ko'rsatamiz." },
      ...
      belarusScenarioOptions: {
        ...
        already: "Men allaqachon Polshadaman",
      },
      georgiaScenarioOptions: {
      ...
      moldovaScenarioOptions: {
        self: "Moldovadan mustaqil ko'chib o'tyapman",
        already: "Men allaqachon Polshadaman",
      },
      uzbekistanScenarioOptions: {
      ...
      kazakhstanScenarioOptions: {
        self: "Qozog'istondan mustaqil ko'chib o'tyapman",
        already: "Men allaqachon Polshadaman",
      },
      tajikistanScenarioOptions: {
        self: "Tojikistondan mustaqil ko'chib o'tyapman",
        already: "Men allaqachon Polshadaman",
      },
      scenarioStatusOptions: {
        yes: "Ha, karta pobytu yoki amaldagi D vizam bor",
        no: "Yo'q, faqat qisqa muddatli viza yoki vizasiz kirish — hali statusim yo'q",
      },
      hasCarOptions: {
  ...
  tr: {
    ...
    housing: {
      ...
      topDistrictDescs: {
        mokotow: "Fiyat ve kalite açısından en iyi dengelerden biri: prestijli Eski Mokotów'ün villalarının yanında, uluslararası şirketlerin ofislerinin yoğunlaştığı iş merkezi Służewiec yer alır. Gelişmiş altyapı, metro, her zevke uygun parklar ve restoranlar.",
        wola: "Son yılların en dinamik iş ilçesi — Rondo Daszyńskiego'nun bulunduğu gökdelenler ve Varşova'nın yeni iş merkezi. Yine de sessiz wohngebiete ve tarihi binalar korunmuş olup, metro merkeze birkaç dakikada ulaştırır.",
        zoliborz: "Merkezin kuzeysinde yeşil ve sessiz ilçe, savaş öncesi binalar ve parklarla. Varşova'nın en atmosferik ilçelerinden biri, metrosu yakın, konumundan dolayı ortalama üzeri fiyatlar.",
        ochota: "Merkezin yanında ama belirgin şekilde daha ucuz. Sosyalist döneminin çok sayıda konut bloğu, gelişmiş altyapı, yakında Varşova Üniversitesi ve Politeknik — öğrenciler ve genç aileler arasında popüler.",
        srodmiescie: "Varşova'nın tarihi ve iş merkezi — Kültür Sarayı, gökdelenler, müzeler ve restoranlar yürüyüş mesafesinde. Şehirdeki en yüksek fiyatlar, ancak her şey elinizin altında ve mükemmel ulaşım erişimi.",
        ursynow: "Geniş güney ilçesi, çoğunlukla apartmanlar, ancak geniş bulvarlar ve parklar var; SGGW kampüsü yakınında, birçok öğrenci ve genç aile. Merkeze iyi ulaşım — M1 metro hattının son istasyonu.",
        "praga-polnoc": "Tarihi, gerçek Praga — sıvanan fasadlar modaya uygun loftlar ve sanat mekanlarıyla yan yana bulunur. Metro yakın, merkeze yakın, artan yatırımcı ilgisi ve yaratıcı gençlik.",
        "praga-poludnie": "Varşova'nın en kalabalık dzielnica'sı, Vistül'ün sağ kıyısında. Çeşitli yapı — eski Grochów evlerinden metronun yakınındaki yeni konut komplekslerine kadar; daha uygun fiyatlar ve canlı yerel yaşam.",
        bemowo: "Kuzeybatıda konut ilçesi, metro ve geniş Bemowo Parkı ile. Son yılların çok sayıda yeni inşaatı, merkez kadar sessiz, çocuklu aileler için uygun.",
        bielany: "Kuzeybatıda konut ilçesi kendi metro hattı (M1) ile, bol yeşillik ve yakında Kampinos Ormanı. Merkez kadar sessiz, aktif olarak yeni konutlar inşa ediliyor, aileler arasında popüler.",
        targowek: "Vistül'ün sağ kıyısında ilçe, Sovyet döneminin panel binolarıyla yeni konut kompleksleri karışımı. Metro ve tramvaylar, merkezden daha uygun fiyatlar, geliştirilmekte olan altyapı.",
        bialoleka: "Şehrin kuzeyinde en büyük yeni inşaat bölgesi, Varşova'da en uygun fiyatlar. Metro henüz ulaşmamış ancak yapılmakta, altyapı ilçe ile birlikte gelişiyor.",
        wilanow: "Wilanów Sarayı ve Parkı yakınında prestijli güney ilçesi — yeni inşaat, çocuklu aileler, gelişmiş altyapı. Metro yok, ancak iyi otobüs ve tramvay bağlantısı.",
        wlochy: "Şopenhauer Havalimanı yakınında güneybatıda ilçe, sanayi alanları ve yeni konut kompleksleri karışımı. İyi demiryolu bağlantısı (SKM), uygun fiyatlar.",
        rembertow: "Şehrin doğusunda küçük sessiz ilçe, bol yeşillik ve özel konutlar. Merkezden daha uzak, ancak belirgin şekilde daha uygun ve sessiz.",
        wawer: "Varşova'nın alan bakımından en büyük ilçesi, güneydoğu çevresi — özel evler, çam ormanları, yazlık ev atmosferi. Daha zayıf toplu taşıma, ancak uygun fiyatlarla çok alan ve sakinlik.",
        wesola: "Şehrin doğusunda en küçük ve uzak ilçe, neredeyse ayrı bir köy — düşük yapılar, yeşillik, minimum gürültü. Merkeze daha uzun yolculuk, ancak fiyatlar ve huzur telafi eder.",
        ursus: "Şehrin batısında eski sanayi ilçesi, şimdi traktör fabrikasının yerinde yeni konut kompleksleriyle aktif olarak inşa ediliyor. Merkezle iyi demiryolu bağlantısı, bol yeşillik, sessiz atmosfer.",
      },
    ...
    banks: {
      ...
      whichBankDescription: "Yapay zekaya sor",
      whichBankLearnMore: "Öneri al",
      whichBankGuide: { heading: "Hangi banka seçilmeli?" },
      moreAboutBank: "Detaylar",
      ...
      visitStatusCourier: "Kurye ile teslimat",
      visitStatusOnlineIfId: "Polonya kimliğiyle online",
      referralBonusLabel: "Arkadaşını davet et: {amount}",
      branchesByCityLabel: "Şehre göre şube bul",
      usefulPhrasesLabel: "Faydalı ifadeler",
    ...
    medicine: {
      ...
      usefulSites: [
        { url: "znanylekarz.pl", desc: "Online doktor randevusu — Rusça konuşan doktorlar mevcut" },
        { url: "ewus.nfz.gov.pl", desc: "NFZ sigorta durumunuzu kontrol edin" },
        { url: "nfz.gov.pl", desc: "Resmi NFZ web sitesi" },
        { url: "pacjent.gov.pl", desc: "Hasta portalı (IKP): doktor randevuları, e-tarifler, test sonuçları çevrimiçi" },
      ],
      ...
      dentalChains: "Önerilen zincirler: DentalPro, Medicover Stomatologia",
    ...
    guideCard: {
      ...
      headlines: {
        ...
        forForeigners: "Yabancılara elverişli",
      },
      tagDescriptions: {
      ...
      statBranches: "Şubeler",
      statOpeningCost: "Hesap açma ücreti",
      bankHighlights: {
        ...
        "Plus Bank": "Uzaktan bankacılığa odaklı",
    ...
    onboarding: {
      ...
      steps: {
        ...
        kazakhstanScenario: { question: "Bunlardan hangisi size uyuyor?", subheading: "Kazakistan vatandaşları için Polonya'ya vizesiz giriş hiç yok, \"iş\" amacı için ise hızlı oświadczenie süreci değil tam zezwolenie na pracę gerekiyor — başvurular doğrudan Astana'daki büyükelçiliğe veya Almatı'daki konsolosluğa yapılıyor (VFS Global yok), ancak yoğun talepte randevu kura ile belirlenebiliyor ve değerlendirme en az 30 iş günü sürüyor. Doğru yol, sürecin hangi aşamasında olduğunuza bağlı." },
        tajikistanScenario: { question: "Bunlardan hangisi size uyuyor?", subheading: "Polonya'nın Tacikistan'da hiç diplomatik temsilciliği yok — her vize (Schengen C veya ulusal D), her amaç için, sadece şahsen Taşkent'te, Özbekistan'da başvurulabiliyor, çünkü oradaki Polonya büyükelçisi Tacikistan için de akredite. \"İş\" amacı için de tam zezwolenie na pracę gerekiyor — Tacikistan vatandaşları için oświadczenie süreci hiçbir zaman mevcut olmadı. Doğru yol, sürecin hangi aşamasında olduğunuza bağlı." },
        scenarioStatus: { question: "Zaten bir karta pobytu veya geçerli D vizeniz var mı?", subheading: "Evet ise, belgelerinizi yenileme veya güncelleme için normal akışa devam edersiniz. Hayır ise, amaç sorusunu daha sonra soracağız — Polonya'dan çıkmadan yasal statü kazanmanın 2 hazır yolunu göstereceğiz." },
      ...
      moldovaScenarioOptions: {
        self: "Moldova'dan kendi başıma taşınıyorum",
        already: "Zaten Polonya'dayım",
      },
      uzbekistanScenarioOptions: {
      ...
      kazakhstanScenarioOptions: {
        self: "Kazakistan'dan kendi başıma taşınıyorum",
        already: "Zaten Polonya'dayım",
      },
      tajikistanScenarioOptions: {
        self: "Tacikistan'dan kendi başıma taşınıyorum",
        already: "Zaten Polonya'dayım",
      },
      scenarioStatusOptions: {
        yes: "Evet, karta pobytu veya geçerli D vizem var",
        no: "Hayır, sadece kısa süreli vize veya vizesiz giriş — henüz statüm yok",
      },
      results: {
  ...
  tg: {
    ...
    housing: {
      ...
      topDistrictDescs: {
        mokotow: "Яке аз ноҳияҳои калонтарин ва гуногун: Мокотови Қадим бо вилла ҳои престижӣ наздики маркази деловӣ Służewiec қарор дорад, ки сирс офисҳои ширкатҳои байналмилалӣ аст. Инфрасохтори рӯзмарра, метро, боғҳо ва меҳмонхонаҳо аз ҳама намуна.",
        wola: "Ноҳияи деловии динамикии сол-ҳои охирин — осмонkassakoҳо дар Rondo Daszyńskoҷо ва маркази деловии нави Варшава. Дар ҳамин вақт ноҳияҳои хавҳои ором ва сохтмон тарихӣ сақ гашташанд, метро ба марказ дар чанд дақиқа расонед.",
        zoliborz: "Ноҳияи сербарг ва ороми шимали марказ бо сохтмон пеш аз ҷанг ва боғҳо. Яке аз ноҳияҳои ғайб Варшава, метро наздик, нархҳо аз сабаби ҷойгиршуда баланд.",
        ochota: "Наздики Миёнасаҳра, аммо қайд аниқтарин арзан. Бисёр блокҳои туҷҷораҭ аз даври PRL, инфрасохтори рӯзмарра, наздики Донишгоҳи Варшава ва Политехника — дар байни донишҷӯён ва оилаҳои ҷавон маъмул.",
        srodmiescie: "Маркази таржуманӣ ва деловии Варшава — Қасри Фарҳанг, осмонkassakoҳо, музеяҳо ва меҳмонхонаҳо дар фоссили пиёидӣ. Нархҳои баланди шаҳр, аммо ҳаммаи чиз дар даст ва дастрасии нақлиёти фавқулодда.",
        ursynow: "Ноҳияи калони ҷануб, асосан иморатҳои бисёруловӣ, аммо бульварҳо ва боғҳои васеъ дорад; наздики кампуси SGGW, бисёр донишҷӯён ва оилаҳои ҷавон. Пайвастиҳо хуб ба марказ — истгоҳи охирини метро M1.",
        "praga-polnoc": "Таржуманӣ, қуллаи ростини Прага — фасадҳои шикастаашуда наздики лофтҳои заманавӣ ва фазои санъат фассо меканд. Метро наздик, қарибӣ ба марказ, ҷозиба дараҳои сармояаҷӯён ва ҷавонони абдоъӣ.",
        "praga-poludnie": "Дзелницаи Варшава бо аҳолии бешубаҳ, дар соҳили рост Висла. Сохтмони гуногун — аз хонаҳои қадим Грохуув то мироҷашаҳии туҷҷораҭ нави наздики метро; нархҳои демократикӣ ва ҷавони махаллӣ.",
        bemowo: "Ноҳияи туҷҷораҭ дар шимоли ғарб бо метро ва боғи калони Bemowo. Бисёр сохтмони нав аз сол-ҳои охирин, орамионе аз марказ, барои оилаҳо бо фарзандон қулай.",
        bielany: "Ноҳияи туҷҷораҭ дар шимоли ғарб бо шоҳи худ метро (M1), сарҳам фарават ва Ҷангали Kampinos наздик. Орамитаре аз марказ, фаъоло сохтмони хонаҳои нав давом мекард, дар байни оилаҳо маъмул.",
        targowek: "Ноҳияи дар соҳили рост Висла, мешаи сохтмони фалъа совети ва мироҷашаҳии туҷҷораҭ нави. Метро ва трамвайҳо, нархҳо паст аз марказ, инфрасохтори аморезашаванда.",
        bialoleka: "Қитъаи авҷӣ сохтмон дар шимоли шаҳр, нархҳои арзантарини Варшава. Метро то ҳоло наомада, аммо барои сохтман, инфрасохтори бо ноҳия тараққӣ мекард.",
        wilanow: "Ноҳияи престижӣ дар ҷануб наздики Қасри ва Боғи Виланов — сохтмони нав, оилаҳо бо фарзандон, инфрасохтори рӯзмарра. Метро нест, аммо пайвастиҳо хуб бо автобус ва трамвай.",
        wlochy: "Ноҳия наздики Фарудгоҳи Шопен дар ҷануби ғарб, мешаи қитъаҳои саноат ва мироҷашаҳии туҷҷораҭ нави. Пайвастиҳо хуби роҳи ҳан (SKM), нархҳои миёна.",
        rembertow: "Ноҳияи хурди ороми дар шарқи шаҳр, сарҳам фарават ва хонаҳои шахсӣ. Дортаре аз марказ, аммо қайди аниқтарин арзан ва ором.",
        wawer: "Ноҳияи калонтарини Варшава аз луҳи майдон, сарҳади ҷануб-шарқ — хонаҳои шахсӣ, ҷангали сосна, фазои деҳкада. Нақли ҷамоатӣ камтар, аммо фаровон майдон ва ором арзони арзон.",
        wesola: "Ноҳияи хурдтарин ва дураздасте дар шарқи шаҳр, деҳақ алохида — сохтмони паст, сарҳам, садо кам. Доремӣ дортар ба марказ, аммо нархҳо ва оромӣ ҷабро мекунанд.",
        ursus: "Ноҳияи саноатӣ қадим дар ғарби шаҳр, номун актив бо мироҷашаҳии туҷҷораҭ нави дар ҷойи кор-факторӣ трактор сохтанда. Пайвастиҳо хуби роҳи ҳан ба марказ, сарҳам фарават, фазои ором.",
      },
    ...
    banks: {
      ...
      otherBanksLabel: "Рейтинги пурра",
      ...
      whichBankDescription: "Аз ИИ пурсед",
      whichBankLearnMore: "Гирифтани тавсия",
      whichBankGuide: { heading: "Кадом бонкро интихоб кунам?" },
      moreAboutBank: "Бештар",
      moreFiltersBtn: "Боз",
      ...
      visitStatusCourier: "Расонидан бо курер",
      visitStatusOnlineIfId: "Бо шиноснома онлайн",
      referralBonusLabel: "Дӯстро даъват кунед: {amount}",
      branchesByCityLabel: "Филиалҳо аз рӯи шаҳрҳо",
      usefulPhrasesLabel: "Ибораҳои муфид",
    ...
    medicine: {
      ...
      usefulSites: [
        { url: "znanylekarz.pl", desc: "Навишта шудан ба назди духтур онлайн — духтурони русзабон мавҷуданд" },
        { url: "ewus.nfz.gov.pl", desc: "Санҷиши суғуртаи NFZ" },
        { url: "nfz.gov.pl", desc: "Сомонаи расмии NFZ" },
        { url: "pacjent.gov.pl", desc: "Портали бемор (IKP): навишт ба назди духтур, рецептҳои электронӣ, натиҷаҳои санҷиш онлайн" },
      ],
      ...
      dentalChains: "Шабакаҳои тавсияшуда: DentalPro, Medicover Stomatologia",
    ...
    guideCard: {
      ...
      tagsLabel: "Тегҳо",
      tags: { noPesel: "Бе PESEL", fullyOnline: "Комилан онлайн", free: "Ройгон", multicurrency: "Бисёрасъорӣ", forForeigners: "Барои хориҷиён" },
      headlines: {
        noPesel: "Бе PESEL",
        fullyOnline: "Кушодани ҳисоб онлайн",
        free: "Хизматрасонии ройгон",
        multicurrency: "Ҳисоби бисёрасъорӣ",
        forForeigners: "Барои хориҷиён мувофиқ",
      },
      tagDescriptions: {
      ...
      statClients: "Мизоҷон",
      statBranches: "Шӯъбаҳо",
      statOpeningCost: "Кушодани ҳисоб",
      bankHighlights: {
        ...
        "Plus Bank": "Таваҷҷуҳ ба хизматрасонии дурдаст",
    ...
    onboarding: {
      ...
      steps: {
        ...
        kazakhstanScenario: { question: "Кадоме аз инҳо ба шумо мувофиқ аст?", subheading: "Барои шаҳрвандони Қазоқистон вуруди бидуни виза ба Полша тамоман вуҷуд надорад, барои мақсади «кор» бошад zezwolenie na pracę пурра лозим аст (бе бақайдгирии зуди oświadczenie) — ҳуҷҷатҳо бевосита ба сафорат дар Астана ё консулгарӣ дар Алмаато супорида мешаванд (VFS Global нест), аммо ҳангоми талаботи баланд сабти навбат тавассути қуръа муайян мешавад ва баррасӣ на камтар аз 30 рӯзи корӣ давом мекунад. Роҳи дуруст ба марҳилае, ки шумо дар он ҳастед, вобаста аст." },
        tajikistanScenario: { question: "Кадоме аз инҳо ба шумо мувофиқ аст?", subheading: "Полша дар Тоҷикистон умуман намояндагии дипломатӣ надорад — ҳар як виза (шенгении C ё миллии D), барои ҳар мақсад, танҳо шахсан дар Тошканд, Ӯзбекистон супорида мешавад, зеро сафири Полша дар он ҷо ба Тоҷикистон низ аккредита шудааст. Барои мақсади «кор» низ zezwolenie na pracę пурра лозим аст — тартиби oświadczenie барои шаҳрвандони Тоҷикистон ҳеҷ гоҳ дастрас набуд. Роҳи дуруст ба марҳилае, ки шумо дар он ҳастед, вобаста аст." },
        scenarioStatus: { question: "Оё шумо аллакай карта побыту ё визаи D-и амалкунанда доред?", subheading: "Агар ҳа бошад, шумо тавассути раванди муқаррарии навсозӣ ё тамдиди ҳуҷҷатҳо давом медиҳед. Агар не бошад, саволи мақсадро баъдтар мепурсем — 2 роҳи тайёри легализатсия бе баромадан аз Полша нишон медиҳем." },
      ...
      belarusScenarioOptions: {
        ...
        already: "Ман аллакай дар Полша ҳастам",
      },
      georgiaScenarioOptions: {
      ...
      moldovaScenarioOptions: {
        self: "Мустақилона аз Молдова кӯчида истодаам",
        already: "Ман аллакай дар Полша ҳастам",
      },
      uzbekistanScenarioOptions: {
      ...
      kazakhstanScenarioOptions: {
        self: "Мустақилона аз Қазоқистон кӯчида истодаам",
        already: "Ман аллакай дар Полша ҳастам",
      },
      tajikistanScenarioOptions: {
        self: "Мустақилона аз Тоҷикистон кӯчида истодаам",
        already: "Ман аллакай дар Полша ҳастам",
      },
      scenarioStatusOptions: {
        yes: "Ҳа, карта побыту ё визаи D-и амалкунанда дорам",
        no: "Не, танҳо визаи кӯтоҳмуддат ё вуруди бидуни виза — статус ҳанӯз надорам",
      },
      results: {
  ...
  uk: {
    ...
    housing: {
      ...
      topDistrictDescs: {
        mokotow: "Один з найбільших і найрізноманітніших районів: престижний Старий Мокотув з його віллами прилягає до ділового Служевця, де зосереджені офіси міжнародних компаній. Розвинута інфраструктура, метро, парки та ресторани на любий смак.",
        wola: "Найдинамічніший діловий район останніх років — хмарочоси біля Rondo Daszyńskiego та новий діловий центр Варшави. При цьому збереглися тиха житлові квартали та історична забудова, метро зв'язує з центром за кілька хвилин.",
        zoliborz: "Зелений і спокійний район на північній стороні центру з довоєнною забудовою та парками. Один з найатмосферніших районів Варшави, метро поблизу, ціни вище середнього через розташування.",
        ochota: "Межує зі Śródmieściem, але помітно дешевше. Багато кварталів забудови часів ПНР, розвинута інфраструктура, поруч Варшавський університет та Політехніка — популярно у студентів та молодих сімей.",
        srodmiescie: "Історичний і діловий центр Варшави — Дворец культури, хмарочоси, музеї та ресторани в межах пішохідної доступності. Найвищі ціни в місті, але все під рукою й чудова транспортна доступність.",
        ursynow: "Великий південний район, переважно багатоповерхові будинки, але з широкими бульварами й парками; поруч з кампусом SGGW, багато студентів та молодих сімей. Гарний зв'язок з центром — кінцева станція метро M1.",
        "praga-polnoc": "Історична, справжня Прага — облуплені фасади сусідять з модними лофтами та арт-простором. Метро поруч, близько до центру, зростаючий інтерес інвесторів та творчої молоді.",
        "praga-poludnie": "Найбільш населена dzielnica Варшави на правому березі Вісли. Різноманітна забудова — від старих будинків Грохова до нових житлових комплексів біля метро; більш демократичні ціни й жива місцева жизнь.",
        bemowo: "Житловий район на північному заході з метро та великим парком Bemowo. Багато новобудов останніх років, спокійніше центру, зручно для сімей з дітьми.",
        bielany: "Житловий район на північному заході зі своєю гілкою метро (M1), багато зелені та Kampinos Forest поруч. Спокійніше центру, активно будуються нові житла, популярні у сімей.",
        targowek: "Район на правому березі Вісли, суміш панельної забудови радянських часів та нових житлових комплексів. Метро та трамваї, ціни нижче, ніж у центрі, розвивається інфраструктура.",
        bialoleka: "Найбільша зона новобудів на півночі міста, найнижчі ціни у Варшаві. Метро ще не дійшло, але будується, інфраструктура розвивається разом з районом.",
        wilanow: "Престижний південний район поруч з палацом і парком Вілянова — нові будівництво, сім'ї з дітьми, розвинута інфраструктура. Немає метро, але хорошим зв'язок автобусом і трамваєм.",
        wlochy: "Район поруч з аеропортом Шопена на південному заході, суміш промислових зон та нових житлових комплексів. Хорошм залізничне сполучення (SKM), помірні ціни.",
        rembertow: "Невеликий спокійний район на сході міста, багато зелені та приватних будинків. Далі від центру, але помітно дешевше й спокійніше.",
        wawer: "Найбільший район Варшави за площею, південно-східна окраїна — приватні будинки, соснові ліси, дачна атмосфера. Громадський транспорт слабший, але багато простору й спокою за доступні гроші.",
        wesola: "Найменший та найдальший район на сході міста, майже окремий село — низька забудова, озеленення, мало шуму. Дороге до центру, але ціни й спокій компенсують.",
        ursus: "Колишній промисловий район на західній стороні міста, нинішнім активно будується новими житловими комплексами на місці тракторного заводу. Хорошм залізничне сполучення з центром, багато зелені, спокійна атмосфера.",
      },
    ...
    banks: {
      ...
      otherBanksLabel: "Повний список рейтингу",
      ...
      whichBankDescription: "Запитати ШІ",
      whichBankLearnMore: "Отримати рекомендацію",
      whichBankGuide: { heading: "Який банк обрати?" },
      moreAboutBank: "Детальніше",
      moreFiltersBtn: "Ще",
      ...
      visitStatusCourier: "Доставка кур'єром",
      visitStatusOnlineIfId: "Онлайн з польським ID",
      referralBonusLabel: "Запроси друга: {amount}",
      branchesByCityLabel: "Відділення по містах",
      usefulPhrasesLabel: "Корисні фрази",
    ...
    medicine: {
      ...
      usefulSites: [
        { url: "znanylekarz.pl", desc: "Запис до лікаря онлайн — є російськомовні лікарі" },
        { url: "ewus.nfz.gov.pl", desc: "Перевірити свою страховку NFZ" },
        { url: "nfz.gov.pl", desc: "Офіційний сайт NFZ" },
        { url: "pacjent.gov.pl", desc: "Особистий кабінет пацієнта (IKP): запис до лікаря, електронні рецепти, результати аналізів онлайн" },
      ],
      ...
      dentalChains: "Рекомендовані мережі: DentalPro, Medicover Stomatologia",
    ...
    guideCard: {
      ...
      headlines: {
        ...
        forForeigners: "Лояльний до іноземців",
      },
      tagDescriptions: {
      ...
      statClients: "Клієнти",
      statBranches: "Відділення",
      statOpeningCost: "Відкриття рахунку",
      bankHighlights: {
        ...
        "Plus Bank": "Акцент на дистанційне обслуговування",
    ...
    onboarding: {
      ...
      steps: {
        ...
        kazakhstanScenario: { question: "Який із варіантів вам підходить?", subheading: "Безвізового в'їзду до Польщі для громадян Казахстану немає взагалі, а для мети «робота» потрібне повне zezwolenie na pracę (без швидкої процедури oświadczenie) — документи подаються напряму в посольство в Астані або консульство в Алмати (візового центру немає), але за високого попиту запис відбувається через жеребкування, а сам розгляд триває від 30 робочих днів. Потрібний шлях залежить від того, на якому етапі ви вже перебуваєте." },
        tajikistanScenario: { question: "Який із варіантів вам підходить?", subheading: "Польща взагалі не має дипломатичного представництва в Таджикистані — будь-яка віза (шенгенська C чи національна D), для будь-якої мети, подається особисто лише в Ташкенті, Узбекистан, оскільки польський посол там акредитований і на Таджикистан. Для мети «робота» також потрібне повне zezwolenie na pracę — процедура oświadczenie для громадян Таджикистану ніколи не була доступна. Потрібний шлях залежить від того, на якому етапі ви вже перебуваєте." },
        scenarioStatus: { question: "У вас вже є карта побиту або чинна віза D?", subheading: "Якщо так — далі звичайний маршрут з продовження чи оновлення документів. Якщо ні — питання про мету запитаємо пізніше: одразу покажемо 2 готові шляхи легалізації без виїзду з Польщі." },
      ...
      moldovaScenarioOptions: {
        self: "Переїжджаю самостійно з Молдови",
        already: "Я вже в Польщі",
      },
      uzbekistanScenarioOptions: {
      ...
      kazakhstanScenarioOptions: {
        self: "Переїжджаю самостійно з Казахстану",
        already: "Я вже в Польщі",
      },
      tajikistanScenarioOptions: {
        self: "Переїжджаю самостійно з Таджикистану",
        already: "Я вже в Польщі",
      },
      scenarioStatusOptions: {
        yes: "Так, є карта побиту або чинна віза D",
        no: "Ні, лише короткострокова віза або безвіз — статусу ще немає",
      },
      results: {
